    ...
    const handleDownload = async () => {
        try {
            const response = await axios.get('http://localhost:8080/get-files', {
                responseType: 'blob', // penting untuk menangani jenis konten file
            });

            // Membuat URL dari blob response
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'filename.ext'); // Format nama file sesuai kebutuhan
